...
export default function Review() {
    ...
    const [showNotification, setShowNotification] = useState({ display: false, text: '', status: '' } as NotificationType);
    ...
    async function handleUpdateTimesheet(updatedTimesheet: TimesheetInterface) {
        try {
            ...
            setShowNotification({ display: true, text: "Timesheet Data Saved Successfully", status: 'success', icon: notificationIcon.success, bgColor: notificationColor.success.bgColor, progressBarColor: notificationColor.success.progressBarColor });
            setTimeout(() => {
                setShowNotification({ display: false, text: "", status: '' });
            }, 3000)
            return
            /* let updatedTimesheetEntryCollection: TimesheetEntry[] = timesheet.entryCollection.map((timesheetEntry: TimesheetEntry) => {
                if (timesheetEntry.id == updatedTimesheetEntryFormData.id) {
                    ...
                    <div className="section-body">
                        <div className="toast-notification-container">
                            {showNotification.display ?
                                <div className={`toast-notification fixed w-80 top-6 right-6 p-2 rounded shadow-md ease-in-out opacity-90 animate-slideInRight ${'animate-fadeOut'} z-10 ${showNotification.bgColor ?? 'bg-slate-100'} `}>
                                    <div className="px-2 py-2">
                                        <div className="flex gap-x-2">
                                            ...
                                            <div className="message">
                                                <p className="text-sm font-bold">{showNotification.text}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className={`toast-progress absolute block bottom-0 left-0 h-1 w-full animate-progress ${showNotification.progressBarColor ?? 'bg-slate-700'}`}></div>
                                </div>
                                : ''
                            }
                        </div>
