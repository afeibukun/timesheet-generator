...
import { PrimitiveTimesheetEntry, PrimitiveTimesheetEntryError } from "@/lib/types/primitive"
import { useState } from "react"

type EntryEditViewProps = {
    primitiveEntry: PrimitiveTimesheetEntry,
    uiElementId: string,
    updatePrimitiveEntryInRecord: Function,
    deletePrimitiveEntryInRecord: Function,
    entryError: PrimitiveTimesheetEntryError | undefined
}

export default function EntryEditView({ primitiveEntry, uiElementId, updatePrimitiveEntryInRecord, deletePrimitiveEntryInRecord, entryError }: EntryEditViewProps) {
    ...
    const [localPrimitiveEntry, setLocalPrimitiveEntry] = useState(primitiveEntry);

    function handleLocalPrimitiveEntryChange(e: any, itemKey: string) {
        let itemValue = e.target.value;
        if (itemKey === 'hasPremium') itemValue = !!e.target.checked
        const _updatedLocalPrimitiveEntry = { ...localPrimitiveEntry, [itemKey]: itemValue }
        setLocalPrimitiveEntry(_updatedLocalPrimitiveEntry);
        updatePrimitiveEntryInRecord(_updatedLocalPrimitiveEntry)

    }

    const totalHoursInEntry = () => {
        try {
            const _totalHoursInString = TimesheetEntry.getTotalHoursInPrimitiveTimesheetEntry(localPrimitiveEntry);
    ...
    function handleEntryDelete(e: any, entryId: number) {
        deletePrimitiveEntryInRecord(entryId);
    }

    return (
        <div className="entry-body grid grid-cols-12 gap-x-1.5 mb-4" key={localPrimitiveEntry.id}>
            <div className="time-type-edit col-span-2">
                <select
                    ...
                    value={localPrimitiveEntry.entryTypeSlug}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'entryTypeSlug')} >
            ...
            <div className="location-type-edit">
                <select
                    ...
                    className={`w-full text-xs p-1 rounded-sm border ${(localPrimitiveEntry.locationType == LocationType.onshore || localPrimitiveEntry.locationType == LocationType.offshore) ? 'border-black' : 'border-red-600 focus:outline-red-600'}`}
                    value={localPrimitiveEntry.locationType}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'locationType')}>
                    <option value={LocationType.onshore}>Onshore</option>
                    <option value={LocationType.offshore}>Offshore</option>
                </select>
                <>{!(localPrimitiveEntry.locationType == LocationType.onshore || localPrimitiveEntry.locationType == LocationType.offshore) ?
                    <div className="w-full relative">
            ...
            <div className="premium-select place-self-center">
                <input
                    ...
                    title="Check Premium"
                    checked={!!localPrimitiveEntry.hasPremium} onChange={(e) => handleLocalPrimitiveEntryChange(e, 'hasPremium')}
            ...
            <div className="start-time-edit">
                <input
                    ...
                    value={localPrimitiveEntry.entryPeriodStartTime}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'entryPeriodStartTime')}
                    step={300}
            ...
            <div className="finish-time-edit">
                <input
                    ...
                    title="Pick Your Finish Time" value={localPrimitiveEntry.entryPeriodFinishTime}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'entryPeriodFinishTime')}
            ...
            <div className="break-start-time-edit">
                <input
                    ...
                    value={localPrimitiveEntry.breakPeriodStartTime}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'breakPeriodStartTime')}
                    step="300"
            ...
            <div className="break-finish-time-edit">
                <input
                    ...
                    value={localPrimitiveEntry.breakPeriodFinishTime}
                    onChange={(e) => handleLocalPrimitiveEntryChange(e, 'breakPeriodFinishTime')}
                    step="300"
            ...
            <div className="comment-edit col-span-2">
                <input
                    type="text"
                    name="comment"
                    id={`comment-${uiElementId}`} title="Type Your Comment" className="w-full text-xs p-1 rounded-sm border border-black" value={primitiveEntry.comment} onChange={(e) => handleLocalPrimitiveEntryChange(e, 'comment')} />
            </div>
            <div className="action-list">
                <div className="flex gap-x-1 justify-end">
                    ...
                    <button type="button" title="Delete Entry" className="p-0.5 text-red-700" onClick={(e) => handleEntryDelete(e, primitiveEntry.id)}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
